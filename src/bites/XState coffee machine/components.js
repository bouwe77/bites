...
export const CoffeeMachine = ({ children }) => {
  return (
    <CoffeeMachineProvider>
      <CoffeeMachineContainer>{children}</CoffeeMachineContainer>
    </CoffeeMachineProvider>
  )
}

const CoffeeMachineContainer = ({ children }) => {
  const { reservoirInMachine } = useCoffeeMachine()
...
export const WaterReservoir = ({ contents = 0 }) => {
  const [showButtons, setShowButtons] = React.useState(false)
  const { reservoirInMachine, toggleReservoirInMachine } = useCoffeeMachine()
  ...
  return (
    ...
    >
      ...
      >
        ...
        <button style={{ width: '80px', height: '40px', margin: '20px' }}>Fill</button>
      </div>
      <div
        style={{
...
const CoffeeMachineContext = React.createContext()

function CoffeeMachineProvider({ children }) {
  const [reservoirInMachine, setReservoirInMachine] = React.useState(true)

  const toggleReservoirInMachine = () => {
    setReservoirInMachine((reservoirInMachine) => !reservoirInMachine)
  }

  return (
    <CoffeeMachineContext.Provider value={{ reservoirInMachine, toggleReservoirInMachine }}>
      {children}
...
function useCoffeeMachine() {
  const context = React.useContext(CoffeeMachineContext)
  if (context === undefined) {
    throw new Error('useCoffeeMachine must be used within a CoffeeMachineProvider')
  }
  return context
}
